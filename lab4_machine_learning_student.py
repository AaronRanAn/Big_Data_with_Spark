...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avg = float(total) / len(IDandRatingsTuple[1])
    
    final = (IDandRatingsTuple[0],(length, avg))
...
getCountsAndAverages((1, (1, 2, 3, 4))), (1, (4, 2.5))
...
movieNameWithAvgRatingsRDD = (moviesRDD.rightOuterJoin(movieIDsWithAvgRatingsRDD)
                                       .map(lambda (k, (V1, (V2, V3))): (V3, V1, V2)))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (k,v):(k,((v[0]-v[1])**2))))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda ((a,b),c):c).reduce(lambda a,b: a+b)
...
trainingTotRating = trainingRDD.map(lambda (a,b,c): c).reduce(lambda a,b: a+b)

trainingCount = trainingRDD.count()

trainingAvgRating = trainingTotRating / trainingCount
...
movieCountsRDD.take(3) # MovieID, number of ratings
...
predictedRDD.take(3) # Movie ID, Predicted Rating
